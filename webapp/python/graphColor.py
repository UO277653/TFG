...
def metodoSimuladorLocal(qp):
    ...
    qaoa = QAOA(optimizer = COBYLA(), quantum_instance=quantum_instance , reps = numeroRep)
...
def metodoSimuladorReal(qp):
    ...
    provider = IBMQ.load_account()
    qaoa_client = QAOAClient(provider=provider , backend=provider.get_backend("ibm_lagos"), reps=numeroRep)
